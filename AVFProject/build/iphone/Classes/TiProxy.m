...
//Rare exceptions to indicate a bug in the _imlocal code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_imlocal = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_imlocal]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.imlocal.module"
