...
//Rare exceptions to indicate a bug in the _project3avf code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_project3avf = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_project3avf]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.project3avf.module"
